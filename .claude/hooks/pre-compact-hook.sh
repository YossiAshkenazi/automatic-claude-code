#!/bin/bash
# Pre-Compact Hook for Agents Observability
...
if command -v jq >/dev/null 2>&1; then
    ...
    trigger=$(echo "$input" | grep -o '"trigger":"[^"]*"' | sed 's/"trigger":"\([^"]*\)"/\1/')
    custom_instructions=$(echo "$input" | grep -o '"custom_instructions":"[^"]*"' | sed 's/"custom_instructions":"\([^"]*\)"/\1/')
...
escaped_trigger=$(echo "$trigger" | sed 's/\\/\\\\/g; s/"/\\"/g; s/\n/\\n/g; s/\r/\\r/g; s/\t/\\t/g')
escaped_custom_instructions=$(echo "$custom_instructions" | sed 's/\\/\\\\/g; s/"/\\"/g; s/\n/\\n/g; s/\r/\\r/g; s/\t/\\t/g')
