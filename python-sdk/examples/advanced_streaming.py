...
class CustomStreamHandler(StreamingHandler):
    ...
    async def handle_result(self, message: ResultMessage):
        ...
        print(f"\n[RESULT] Final Result (after {elapsed:.1f}s):")
        print(f"[NOTE] Content: {message.result[:200]}...")
    ...
    async def handle_tool_use(self, message: ToolUseMessage):
        ...
        print(f"\n[TOOL] Tool: {tool_name} (used {self.tool_usage[tool_name]} times)")
...
async def example_1_custom_handler():
    ...
    try:
        ...
        print(f"[STATS] Tool usage summary: {handler.tool_usage}")
...
async def example_2_message_collector():
    ...
    try:
        ...
        print(f"[NOTE] Results: {len(results)}")
        print(f"[FAIL] Errors: {len(errors)}")
        print(f"[TOOL] Tool uses: {len(tool_uses)}")
        
        if results:
            print(f"[RESULT] Final result preview: {results[-1].result[:150]}...")
        ...
        print(f"[FAIL] Collection failed: {e}")
...
async def example_3_filtered_streaming():
    ...
    try:
        ...
        async for result_message in result_only_filter(
            ...
            print(f"[OK] Got result: {len(result_message.result)} characters")
        
        print(f"[STATS] Collected {len(results)} result messages")
        ...
        async for error_message in error_only_filter(
            ...
            error_count += 1
            print(f"[FAIL] Error {error_count}: {error_message.error}")
        
        if error_count == 0:
            print("[OK] No errors detected (this is good!)")
...
async def example_4_real_time_processing():
    ...
    class RealTimeProcessor:
        ...
        async def process_stream(self, message):
            if isinstance(message, StreamMessage) and message.content:
                ...
                elapsed = time.time() - self.start_time
                print(f"\n[RESULT] Final: {self.word_count} words in {elapsed:.1f}s")
    ...
    try:
        ...
        if result:
            print(f"[NOTE] Result length: {len(result)} characters")
    
    except Exception as e:
        print(f"[FAIL] Real-time processing failed: {e}")
    ...
    try:
        ...
        async for message in query("Create a Python class for managing a simple database connection"):
            ...
            if monitor.message_count % 5 == 0:
                ...
                print(f"\r[STATS] Messages: {monitor.message_count}, Time: {elapsed:.1f}s", end="", flush=True)
...
async def example_6_utility_functions():
    ...
    try:
        ...
        print(f"[STATS] Collected {len(messages)} total messages")
        ...
        if result:
            print(f"[RESULT] Final result: {result[:200]}...")
        else:
...
async def main():
    ...
    # Summary
    print("\n" + "=" * 60)
    print("[STATS] Advanced Streaming Examples Summary")
    print("=" * 60)
    ...
    if successful == len(examples):
        ...
        print("[WARN] Most examples completed. Check any error messages above.")
    else:
