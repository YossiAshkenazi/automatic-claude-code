...
class LogViewer {
    ...
    getSessionSummaries() {
        ...
        for (const logFile of logFiles) {
            ...
            // Get initial prompt
            const initialPromptEntry = entries.find(e => e.details && typeof e.details === 'object' && e.details.initialPrompt);
            ...
            const status = hasEndMarker ? 'completed' :
    ...
    displaySessionSummary(summaries) {
        if (summaries.length === 0) {
            console.log(chalk_1.default.yellow('No sessions found.'));
            return;
        }
        console.log((0, boxen_1.default)(chalk_1.default.bold.blue('📊 Session Overview'), { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
    ...
    displaySessionDetails(sessionId) {
        ...
        const entries = this.parseLogFile(filePath);
        if (entries.length === 0) {
            console.log(chalk_1.default.red(`No entries found for session: ${sessionId}`));
            return;
        }
        // Session header
        ...
        console.log((0, boxen_1.default)(chalk_1.default.bold.blue(`📋 Session: ${sessionId}\n`) +
            chalk_1.default.cyan(`Task: ${initialPrompt}\n`) +
            chalk_1.default.gray(`Started: ${entries[0].timestamp.toLocaleString()}`), { padding: 1, borderColor: 'blue', borderStyle: 'double' }));
        ...
        if (otherEntries.length > 0) {
            console.log(chalk_1.default.yellow.bold('📝 Session Setup:'));
        ...
        sortedIterations.forEach(iteration => {
            ...
            if (errors > 0)
                ...
                statusIcon = '⚠️';
            else if (iterEntries.some(e => e.level === 'SUCCESS'))
                statusIcon = '✅';
    ...
    displayLogEntry(entry, compact = false) {
        ...
        switch (entry.level) {
            case 'DEBUG':
                levelIcon = '🐛';
                levelColor = chalk_1.default.gray;
                break;
            case 'INFO':
                levelIcon = 'ℹ️';
        ...
        const prefix = compact ?
            ...
            `${chalk_1.default.gray(timestamp)} ${levelIcon} ${levelColor(entry.level.padEnd(8))}`;
        ...
        if (entry.details && !compact) {
            ...
            try {
                const highlighted = (0, cli_highlight_1.highlight)(detailsStr, { language: 'json' });
                console.log(chalk_1.default.gray(highlighted.split('\n').map(line => `    ${line}`).join('\n')));
            }
            catch (e) {
                console.log(chalk_1.default.gray(detailsStr.split('\n').map(line => `    ${line}`).join('\n')));
            }
        }
    }
    // Export to HTML
    exportToHTML(sessionId, outputPath) {
        ...
        console.log(chalk_1.default.green(`✅ HTML report exported to: ${finalPath}`));
    }
    generateSessionHTML(sessionId, task, entries) {
...
<body>
    ...
    searchLogs(searchTerm, sessionId) {
        const results = [];
        if (sessionId) {
            ...
            const filePath = path.join(this.logDir, logFile);
            const entries = this.parseLogFile(filePath);
            return entries.filter(entry => entry.message.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    displayWorkOutput(sessionTimestamp) {
        const workLogs = this.getWorkLogs(sessionTimestamp);
        if (workLogs.length === 0) {
            console.log(chalk_1.default.yellow('No work output found.'));
            return;
        }
        console.log((0, boxen_1.default)(chalk_1.default.bold.blue('📝 Claude Work Output'), { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
        workLogs.forEach(entry => {
            // Skip headers and system messages
            if (entry.message.includes('===') || entry.level !== 'INFO') {
                return;
            }
            console.log(chalk_1.default.white(entry.message));
        });
        console.log();
    }
    // Statistics
