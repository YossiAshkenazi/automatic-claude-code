...
def test_health_checker():
    ...
    try:
        ...
        if port_available:
            ...
            print("   ⚠️  Port 9999 unexpectedly occupied")
        
        # Test component configuration
        if 'python_backend' in checker.COMPONENTS:
            print("   ✓ Component configuration loaded")
        else:
    ...
    try:
        ...
        for script in required_root_scripts:
            if script in root_data.get('scripts', {}):
                print(f"   ✓ Root script '{script}' exists")
            else:
                print(f"   ⚠️  Root script '{script}' missing (optional)")
        ...
        for script in required_dashboard_scripts:
            if script in dashboard_data.get('scripts', {}):
                print(f"   ✓ Dashboard script '{script}' exists")
            else:
                print(f"   ✗ Dashboard script '{script}' missing")
                return False
        
        print("   ✓ Package scripts configuration looks good")
        return True
        
    except Exception as e:
        print(f"   ✗ Error checking package scripts: {e}")
        return False
