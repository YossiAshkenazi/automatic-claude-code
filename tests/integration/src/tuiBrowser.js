...
class TUIBrowser {
    constructor(repoName) {
        this.logViewer = new logViewer_1.LogViewer(repoName);
        this.state = {
            selectedSession: 0,
            selectedIteration: 0,
            viewMode: 'sessions',
    ...
    initializeUI() {
        ...
        this.sessionList = blessed_1.default.list({
            label: ' Sessions ',
            top: 3,
            left: 0,
            width: '40%',
        ...
        this.iterationList = blessed_1.default.list({
            label: ' Iterations ',
            top: 3,
            left: '40%',
            width: '60%',
        ...
        // Progress Bar
        this.progressBar = blessed_contrib_1.default.gauge({
        ...
        // Status Bar
        this.statusBar = blessed_1.default.text({
            bottom: 0,
            left: 0,
            width: '100%',
            height: 3,
    ...
    setupEventHandlers() {
        ...
        this.iterationList.on('select', (item, index) => {
    ...
    loadSessionIterations() {
        ...
        const logViewer = new logViewer_1.LogViewer();
        try {
            this.state.currentSessionLogs = logViewer.parseLogFile(require('path').join(require('os').homedir(), '.automatic-claude-code', 'logs', logViewer['repoName'], logFile));
            // Group by iterations
            ...
            const items = Array.from(iterationGroups.keys())
                ...
                if (errors > 0)
                    ...
                    statusIcon = '{yellow-fg}⚠️{/yellow-fg}';
                else if (successes > 0)
                    statusIcon = '{green-fg}✅{/green-fg}';
    ...
    loadIterationDetails() {
        if (this.state.currentSessionLogs.length === 0)
            return;
        // Group by iterations
        const iterationGroups = new Map();
        this.state.currentSessionLogs.forEach(entry => {
    ...
    filterSessions() {
        if (!this.state.searchQuery.trim()) {
            ...
            this.state.filteredSessions = this.logViewer.getSessionSummaries().filter(session => session.initialPrompt.toLowerCase().includes(query) ||
...
exports.TUIBrowser = TUIBrowser;
// Export function to launch the TUI
function launchTUI(repoName) {
